---
layout: casestudy
title: Leading global standards for inclusive CX
description: Integrating inclusion into customer experience standards for CXPA
project_name: CXPA
category: Creativity
skip_section_icons: true
image: /assets/images/casestudies/dei_cx_publication/DP_Hero.jpg
thumbnail: /assets/images/casestudies/dei_cx_publication/DP_Thumbnail.jpg
challange: "Inclusion and belonging are important for both customer experience and employee experience, **however there was not a credible, global publication addressing this topic.** When considering inclusion and belonging globally, there is no one-size-fits all. **Being mindful of different cultures is key to creating a credible resource.**"  
approach: "Our goal as a team was to write a publication that could **serve as a reference for different situations, knowing companies will approach this topic in different ways.** We wanted to curate different methods, examples and watch-outs **so leaders could apply what was relevant to their teams.**"
responsibilities: [Project Management, Team Management, Committee Engagement, Researching, Writing]
---

{% include section_icons.html 
titles="Team Strategy, Book Structure, Research + Writing, Reviews + Editing" 
icons="Discover, Define, Design, Validate"
breakout=true %}

![](/assets/images/casestudies/dei_cx_publication/Team_Plan.jpg)
{% include img_caption.html 
title="Aligning team structure" 
content="Worked to get the right structure and resources for the 15+ member team." %}

![](/assets/images/casestudies/dei_cx_publication/Pages_Example.jpg)
{% include img_caption.html 
title="Easy-to-read publication" 
content="We used different content blocks and call outs to make this an engaging reference." %}

## Outcome

{% include case_h3_block.html 
title="Learnings +<br>Results" 
content="**We had a successful publication launch at CX Day in October.** Creating a publication structure from the beginning helped to organise the team and focus their efforts. It is **still being used by the CXPA Diversity Advancement Committee as a key resource for conversations, webinars and content.** It has been good to see its longevity in a topic that keeps evolving. " %}

{% capture info-highlight %}
#### Downloaded 300+ times by CXPA members
|
#### First publication of its kind to explore this topic
|
#### Used as a springboard for webinars
{% endcapture %}
{% include info-highlight.html content=info-highlight %}
